...
registered: kflarsen, br0ns, moki, thordenm, and you. The password for
you is mastermind.
...
Your solution is a URL starting with `localhost/barbarbar/users.php?`

 * The grader will already be logged in to the local barbarbar before
   loading your URL.

 * Your goal is to steal the document cookie and make an HTTP GET
   request to
       http://localhost:8080/barbarbar/cookies.php?cookie=...cookie data here...

   Before your attack, start `nc -l 8080` on the command line to see
   the cookie data arrive.

 * Except for the browser address bar (which can be different), the
   grader should see a page that looks exactly as it normally does
   when the grader visits users.php. No changes to the site appearance
   or extraneous text should be visible. Avoiding the red warning text
   is an important part of this attack. (It's ok if the page looks
   weird briefly before correcting itself.)

 * Hint: `/users.php?user="><script>alert(document.cookie);</script>`
   is an example attack to use as a starting point.
...
You shall focus on SQL Injection.  Your main tasks are to:
  ...
  - See if you can change the password for 'br0ns'.
